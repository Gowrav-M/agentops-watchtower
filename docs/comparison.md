...
| MCP firewalls/proxies | Enforce some calls in the runtime path. | Usually require being installed as a proxy before an incident; Watchtower can also reconstruct risk paths from existing local traces and generate protected config wrappers. |
...
agentops-watchtower protect-mcp --config .mcp.json --server github --descriptor tools.json
